...
$query_drapeauPays = sprintf("SELECT ch_pay_emplacement, ch_pay_nom, ch_pay_lien_imgdrapeau FROM pays WHERE ch_pay_id = %s AND ch_pay_publication=1", GetSQLValueString($colname_MarkerVilles, "int"));
$drapeauPays = mysql_query($query_drapeauPays, $maconnexion) or die(mysql_error());
...
$query_VoiesPays = sprintf("SELECT ch_geo_id, ch_geo_wkt, ch_geo_pay_id, ch_geo_user, ch_geo_maj_user, ch_geo_date, ch_geo_mis_jour, ch_geo_geometries, ch_geo_type, ch_geo_nom, ch_geo_mesure, ch_use_login FROM geometries LEFT JOIN pays ON ch_geo_pay_id = ch_pay_id LEFT JOIN users ON ch_geo_user = ch_use_id WHERE (ch_pay_publication = 1 OR ch_geo_pay_id = 1) AND ch_geo_geometries = 'line' AND (ch_geo_pay_id = %s OR ch_geo_pay_id = 1)", GetSQLValueString($colname_MarkerVilles, "int"));
...
?>
		var x = '<?php echo $x; ?>' ;
		var y = '<?php echo $y; ?>' ;  
		var urlicon ='<?php echo $row_drapeauPays['ch_pay_lien_imgdrapeau']; ?>'
                features.push(new OpenLayers.Feature.Vector(
                    new OpenLayers.Geometry.Point(x,y), features.attributes = {
                name: "<?php echo $Nompays; ?>",
				flag: "<?php echo $row_drapeauPays['ch_pay_lien_imgdrapeau']; ?>"
            }));
            return features;
        }
